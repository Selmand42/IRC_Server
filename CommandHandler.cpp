...
void CommandHandler::handleNick(User& user, const std::vector<std::string>& args) {
    ...
    if (!isValidNickname(new_nick)) {
        user.sendMessage(":server 432 " + new_nick + " :Erroneous nickname");
        return;
    }
    ...
    for (it = users.begin(); it != users.end(); ++it) {
        if (it->second.getNickname() == new_nick) {
            user.sendMessage(":server 433 " + new_nick + " :Nickname is already in use");
            return;
        }
    }
    ...
    if (user.isRegistered()) {
        user.sendMessage(":" + user.getNickname() + " NICK :" + new_nick);
    }
}
